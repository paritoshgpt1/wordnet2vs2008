...
#ifdef _WINDOWS
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, TEXT("Software\\WordNet\\2.1"),
		     0, KEY_READ, &hkey) == ERROR_SUCCESS) {
	dwSize = sizeof(searchdir);
	RegQueryValueExA(hkey, TEXT("WNHome"),
			NULL, &dwType, searchdir, &dwSize);
	RegCloseKey(hkey);
	strcat(searchdir, DICTDIR);
    }
    else if (RegOpenKeyExA(HKEY_CURRENT_USER, TEXT("Software\\WordNet\\2.1"),
		     0, KEY_READ, &hkey) == ERROR_SUCCESS) {
	dwSize = sizeof(searchdir);
	RegQueryValueExA(hkey, TEXT("WNHome"),
			NULL, &dwType, searchdir, &dwSize);
...
char *morphstr(char *origstr, int pos)
{
	...
	dprintf("morphstr(origstr = %s, pos = %d)", origstr, pos);
...
char *morphword(char *word, int pos)
{
	...
	dprintf("morphstr(word = %s, pos = %d)", word, pos);
...
static int strend(char *str1, char *str2)
{
	...
	dprintf("strend(str1 = %s, str2 = %s)", str1, str2);
...
static char *wordbase(char *word, int ender)
{
	...
	dprintf("wordbase(word = %s, ender = %d)", word, ender);
...
static int hasprep(char *s, int wdcnt)
{
	...
	dprintf("hasprep(s = %s, wdcnt = %d)", s, wdcnt);
...
static char *exc_lookup(char *word, int pos)
{
	...
	dprintf("exc_lookup(word = %s, pos = %d)", word, pos);
